...
def add_parser(subparsers):
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            the retrieval results (full distributions) combined with the
            reference values.
            """
        ),
    )
...
def process_dataset(mrnn, data_loader, device="cpu"):
    ...
    with torch.no_grad():
        for x, y in tqdm.tqdm(data_loader, ncols=80):
            ...
            tiwp_distribution.append(y_pred["tiwp"].cpu().numpy())
            ...
            tiwp_fpavg_distribution.append(y_pred["tiwp_fpavg"].cpu().numpy())
            ...
            tiwc_distribution.append(y_pred["tiwc"].cpu().numpy())
    ...
    tiwp_distribution = np.concatenate(tiwp_distribution, axis=1).T
    ...
    tiwp_fpavg_distribution = np.concatenate(tiwp_fpavg_distribution, axis=1).T
    ...
    tiwc_distribution = np.concatenate(tiwc_distribution, axis=2).transpose((2,1,0))
    ...
    dataset.tiwp_distribution.encoding = enc_float
